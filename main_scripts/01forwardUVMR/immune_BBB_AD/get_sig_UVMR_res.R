get_UVMR_sig <- function(input = input )
{
  message("input = output of get_UVMR_collected function, a dataframe ")
  #significant association criteria:
  ##1. cML as primary analysis for quantify the effect due to its robustness to both correlated and uncorrelated pleiotropic effects and consideration of potential bais of population stratification and sample overlaping, so that pval_cML-MA-BIC < 0.05;
  ...
  out<- out %>% 
        dplyr::filter(`pval_cML-MA-BIC` < 0.05 & n_significant_simpleout >2 & (all_positive == TRUE | all_negative == TRUE) ) %>%
