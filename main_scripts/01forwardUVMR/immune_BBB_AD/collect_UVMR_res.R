get_UVMR_collected <- function(inputpath = inputpath,
                              outpath = outpath,
                              outfilename = outfilename)
{
  ...
  message("outfilename = outfilename, character ")
  ...
  inputpath <- inputpath
  outlist <- list.files(inputpath,recursive = TRUE, pattern = "\\.xlsx$",full.names = T)
###
  for(i in 1:length(outlist))
  {
    setwd(inputpath)
    if(!file.exists(outlist[i])) next
    else
    {
        ### F statistics and Isq
        F_sheet <- read.xlsx(outlist[i], sheet=3) 
        F <- mean(F_sheet$F_statistic)
        I_sheet <-   read.xlsx(outlist[i], sheet=9)
        Isq <- I_sheet[1,1]
        F_I <- data.frame(F_statistic=F,Isq=Isq)

        ##7MR
        seven_mr <- read.xlsx(outlist[i], sheet=2) 
        
        ###If there exists features with less than 3 SNPs and not all 7 methods are done, therefore each cell should be manually added as "-"
        ...
        pleiotropy <- read.xlsx(outlist[i], sheet=4) %>% dplyr::select(c("egger_intercept","se","pval"))
    
        heterogeneity1 <- read.xlsx(outlist[i], sheet=5) %>%
           tidyr::pivot_wider(., id_cols=c("id.outcome","outcome","exposure"),names_from="method",names_vary="slowest",
                        values_from=c("Q","Q_df","Q_pval"))

        MRPRESSO_global <- read.xlsx(outlist[i], sheet=6) %>% slice_tail(n=1) %>% dplyr::select(c("globalP"))
    
        reverseMR <-  read.xlsx(outlist[i], sheet=8) %>% dplyr::select(c(5:8))
  ...
  out <- out_all_res %>% mutate(n_significant_simplein=rowSums(dplyr::select(.,
                              ...
                              n_significant_simpleout=rowSums(dplyr::select(.,
                                                            `pval_MR Egger`, `pval_Weighted median`, `pval_Inverse variance weighted`,
  ...
  writexl::write_xlsx(x = list("all_main_res"=out),
      path =  paste0(outpath,outfilename,".xlsx"))
