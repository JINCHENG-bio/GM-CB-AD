...
blood_double <- function(i){
  file_list <- list.files("/mnt/data/lijincheng/mGWAS/data/mediator/raw", pattern = "\\.tsv.gz$", full.names = TRUE)
  
  ### id_meta <- id_metabolite  %>% filter(str_detect(`STUDY ACCESSION`, meta)) %>% pull(`DISEASE/TRAIT`);print(id_meta)
  out <- "/mnt/data/lijincheng/mGWAS/result/04replication/Kunkle/metabolites/"
  setwd(out)
  
  meta <- substr(file_list[i],50,61)
  id_metabolite <- fread("/mnt/data/lijincheng/mGWAS/result/01UVMR/02mediator_outcome/metabolite_accession.csv")
  id_meta <- id_metabolite  %>% filter(str_detect(`STUDY ACCESSION`, meta)) %>% pull(`DISEASE/TRAIT`);print(id_meta)
  id <- id_meta
  dir.create(id)
  out_each <- paste0(out,id,"/")
  
  blood <- fread(file_list[i]) 
  blood$p_value <- as.numeric(blood$p_value)
  blood <- blood %>% dplyr::filter(p_value < 5e-8) %>% mutate(N = 115082 , phe = id)
  
  blood <-  blood[order(blood$p_value), ]
  ...
  tryCatch({
   blood_exp_dat <- TwoSampleMR::clump_data(dat = blood_exp_dat,
    ...
    AD_out_dat <-  TwoSampleMR::extract_outcome_data(snps =blood_exp_dat$SNP, outcomes =   'ieu-b-2', #'ebi-a-GCST90012877', #2013lambert  ##'ieu-b-2', ###kunkle
                                       maf_threshold = 0.01)
     ###-----{2}harmonise-----
  ...
  ###------{3}重要封装函数的读取----- 
   ...
   ###-----{5}main_result------
    ...
    print( paste0("double check finished ",i, " !"))
  }, error = function(e) {
    # 处理报错信息
