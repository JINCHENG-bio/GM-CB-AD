snp_replace_proxy <- function(dat, snp_proxy, type = "exposure", build = "37", pop = "EUR")
{
  ...
  index <- ifelse(
    (dat[[paste0("eaf.",type)]]<0.5&ref_af<0.5)|
    (dat[[paste0("eaf.",type)]]>0.5&ref_af>0.5),
    1,2
  )
...
#    print("No appropriate SNP found at that reference panel, skip that SNP!") #An error occurred while fetching data from the web
#    print(e)
#    return(NULL)
#  })
#}